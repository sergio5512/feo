module.exports = async (Discord, client, message, set, db, prefix, blacklist_db, interchat_blacklist, verificados, canalesprivados_db) => {
  if (message.author.bot) return;
    if(message.author.id === '528860743437910016'||message.author.id === '353104236491309056'||message.author.id === '525719983506980903'||message.author.id === '456361646273593345') return;
    ...
    if (message.content.includes('.gg')||message.content.includes('.com')||message.content.includes('http')||message.content.includes('app.')) return;
        ...
        channel.createInvite({ unique: false })
            .then(invite => {
                const embed = new Discord.MessageEmbed()
                    ...
                    .setColor('RANDOM')
                    .setDescription(`:busts_in_silhouette:  [**USUARIO**](https://discord.com/api/oauth2/authorize?client_id=741314725916180480&permissions=8&scope=bot) :busts_in_silhouette:  | [📤 **UNIRSE A ESTE SERVIDOR** 📤](https://discord.gg/${invite.code})`)
